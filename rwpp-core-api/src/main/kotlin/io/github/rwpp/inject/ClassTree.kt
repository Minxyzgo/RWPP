...
class ClassTree(
    ...
    private fun addZipFile(zipFile: ZipFile) {
        zipFile.entries().toList().mapNotNull {
            ...
            val packageName = entry.substring(0, entry.lastIndexOf('.'))
            addPackageTree(packageName)
            val className = entry.substring(entry.lastIndexOf('.') + 1, entry.lastIndex + 1)
            val tree = allPackageFromRoot[packageName]!!
            tree.classesName.add(className)
        }
    }
