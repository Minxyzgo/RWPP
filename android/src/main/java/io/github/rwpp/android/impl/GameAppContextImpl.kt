...
class GameAppContextImpl(private val _exit: () -> Unit)
    : AppContext,
    Game by GameImpl(),
    ...
    ExternalHandler by ExternalHandlerImpl()
{
    ...
    override fun <T> getConfig(group: ConfigGroup, name: String): T {
        if (group == InternalGroups.Game) {
    ...
    override fun setConfig(group: ConfigGroup, name: String, value: Any?) {
        if (group == InternalGroups.Game) {
            val field = fieldCache.getOrPut(name) { SettingsEngine::class.java.getDeclaredField(name) }
