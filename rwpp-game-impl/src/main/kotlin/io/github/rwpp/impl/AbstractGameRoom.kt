    ...
    override var selectedMap: GameMap
        ...
        set(value) {
            if (isHostServer) {
                ...
                val realPath = (
                        when (value.mapType) {
                            io.github.rwpp.game.map.MapType.SkirmishMap -> "maps/skirmish/"
                            io.github.rwpp.game.map.MapType.CustomMap -> "mods/maps/"
                            io.github.rwpp.game.map.MapType.SavedGame -> "saves/"
                            else -> ""
    ...
    override fun sendMessageToPlayer(player: Player?, title: String?, message: String, color: Int) {
        if (player != null && player != localPlayer) {
            player.client?.sendPacketToClient(
                io.github.rwpp.net.packets.GamePacket.getChatPacket(
                    title,
                    message,
                    color
                )
